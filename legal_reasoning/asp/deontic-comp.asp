...
file("deontic-comp.asp", "deontic logic with compensation and conversion", "2026-01-20").

obligation(X) :- obligation(R,X,N).

violation(R,X,N) :- obligation(R,obl(X),N), opposes(Y,X), defeasible(Y).
weakViolation(R,X,N) :- obligation(R,obl(X),N), refuted(X).

violation(X) :- obligation(X), opposes(Y,X), defeasible(Y).
weakViolation(X) :- obligation(X), refuted(X).

terminalViolation(R) :- 
    obligation(R,obl(X),N), violation(R,X,N), not compensate(R,X,_,N).

terminalViolation(R) :-
    obligation(R,obl(X),N), violation(R,X,N), obligationRefuted(R,X,N+1).
